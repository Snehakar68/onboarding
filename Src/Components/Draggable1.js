...
import { MARGIN, getOrder, getPosition } from "../assests/Utils1";
import Animated, {
  runOnJS,
...
const Draggable1 = ({
  ...
  const panGesture = useAnimatedGestureHandler({
    ...
    //   console.log("Positionssss===>>>", positions.value[id], id);
    //   runOnJS(() =>
    //     'worklet',
    //     setImgPosition((prev) => ({ ...prev, id: positions.value[id] }))
    //   );
    // },
    onFinish: () => {
      isGestureActive.value = false;
      console.log("Positionssss===>>>", positions.value[id], id);
      ("worklet");
      runOnJS(setImgPosition)({
        ...imgPosition,
        [id]: positions.value[id]
      });
    },
  });
